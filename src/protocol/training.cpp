...
namespace ssns::protocol {
    namespace {
        ...
        linalg::Matrix sample_normal_batch(std::size_t batch, std::size_t cols, nn::Rng &rng) {
    ...
    StepResult clean_train_step(
        nn::CleanClient &client, const nn::CleanServer &server,
        std::size_t batch_size, std::size_t input_dim,
        nn::Rng &rng, double bimodality_alpha) {
    ...
    StepResult clean_train_step_noisy(
        nn::CleanClient &client, const nn::CleanServer &server,
        std::size_t batch_size, std::size_t input_dim,
        nn::Rng &rng, double bimodality_alpha, double noise_std) {
        linalg::Matrix X = sample_normal_batch(batch_size, input_dim, rng);
        linalg::Matrix Y_true = server.teacher_forward(X);
    ...
    namespace {
        ...
        Plaintext encode_scalar(const Encoder &encoder, const std::array<NTT, NUM_PRIMES> &ntts,
        ...
        Ciphertext encrypt_scalar(const Encoder &encoder, const std::array<NTT, NUM_PRIMES> &ntts,
                                  const PublicKey &pk, double value, double scale, std::mt19937_64 &rng) {
        ...
        double decrypt_scalar(const Encoder &encoder, const std::array<NTT, NUM_PRIMES> &ntts,
                              const Polynomial &s_ntt, const Ciphertext &ct) {
    ...
    ClientPayload client_encrypt(
        const linalg::Matrix &X, const linalg::Matrix &H,
        const linalg::Matrix &Y_pred, const ClientKeys &keys, std::mt19937_64 &fhe_rng) {
...
#pragma omp parallel for schedule(static)
    ...
    ServerResponse server_compute_gradients(
        const ClientPayload &payload, const nn::CleanServer &server,
        const ServerKeys &keys, double bimodality_alpha) {
        ...
        // деление на batch заменено на mul_scalar inv_batch, т к в FHE деление невозможно
...
#pragma omp parallel for schedule(static)
        for (std::size_t k = 0; k < E_count; ++k) {
            ...
            Plaintext yt_pt = encode_scalar(*keys.encoder, *keys.ntts, yt_val, keys.scale, NUM_PRIMES);
        ...
        for (std::size_t h = 0; h < H_cols; ++h) {
            for (std::size_t o = 0; o < Y_cols; ++o) {
                Ciphertext acc = mul_cipher(H_l3[0 * H_cols + h],
                    error_ct[0 * Y_cols + o], *keys.evk, *keys.ntts);
                for (std::size_t i = 1; i < batch_size; ++i) {
                    Ciphertext term = mul_cipher(H_l3[i * H_cols + h],
                        error_ct[i * Y_cols + o], *keys.evk, *keys.ntts);
...
#pragma omp parallel for collapse(2) schedule(static)
        for (std::size_t i = 0; i < batch_size; ++i) {
            for (std::size_t h = 0; h < H_cols; ++h) {
                Ciphertext acc = mul_plain(error_ct[i * Y_cols + 0], bfa_pt[0 * H_cols + h]);
                for (std::size_t o = 1; o < Y_cols; ++o) {
                    Ciphertext term = mul_plain(error_ct[i * Y_cols + o], bfa_pt[o * H_cols + h]);
                    acc = add(acc, term);
                }
    ...
    DecryptedGradients client_decrypt(const ServerResponse &response, const ClientKeys &keys) {
...
#pragma omp parallel for collapse(2) schedule(static)
    ...
    StepResult clean_train_step_fhe(nn::CleanClient &client, const nn::CleanServer &server,
                                    ckks::Backend &backend, std::size_t batch_size,
                                    std::size_t input_dim, nn::Rng &rng, double bimodality_alpha) {
