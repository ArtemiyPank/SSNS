...
namespace ssns::linalg {
    namespace {
        ...
        // K_BLOCK * N_BLOCK + M_BLOCK * N_BLOCK + M_BLOCK * K_BLOCK = 320 KBytes < 512 KBytes
        ...
        // avx2 6x8 kernel: A_k(6 * kc) x B_k(kc * 8) = C_k(6 * 8)
        // C += A_panel * B_panel
        ...
        // ld a/b/c - Leading Dimension(длина строки исходной матрицы)
        ...
        // scalar fallback: A_k(mr * kc) x B_k(kc * nr) = C_k(mr * nr)
        ...
        inline void macro_tile(const double *A, std::size_t lda,
                               ...
                               std::size_t mc, std::size_t nc, std::size_t kc, bool use_avx2) {
...
#if SSNS_HAS_X86_INTRIN
                    if (use_avx2) {
                        microkernel_6x8(A + i * lda, lda, B + j, ldb, C + i * ldc + j, ldc, kc);
                        continue;
                    }
#else
            (void)use_avx2;
#endif
                    microkernel_scalar(A + i * lda, lda, B + j, ldb, C + i * ldc + j, ldc, kc, MR, NR);
                }
                // right tail along N
                if (j < nc) {
                    microkernel_scalar(A + i * lda, lda, B + j, ldb, C + i * ldc + j, ldc, kc, MR, nc - j);
                }
            }
    ...
    void matmul_native(const double *A, const double *B, double *C,
        ...
        for (std::size_t t = 0; t < n_threads; t++) {
            const std::size_t this_blocks = blocks_per_worker + (t < blocks_remainder ? 1 : 0);
