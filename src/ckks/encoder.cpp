...
namespace ssns::ckks {
    ...
    Encoder::Encoder() {
        ...
        // вычисляем zeta_k для всех k в [0, N] ζ
        zeta_pow_.resize(N);
    ...
    void Encoder::bitreverse_permute(std::vector<std::complex<double> > &a) const {
        ...
        const int log_N = std::bit_width(N) - 1; // len in bits
    ...
    // Cooley–Tukey FFT algorithm (O(N\log_2 N))
    ...
    Polynomial Encoder::encode(const std::vector<std::complex<double> > &z, double scale) const {
        ...
        for (std::size_t j = 0; j < N; ++j) {
            ...
            const bool is_negative = (scaled < 0.0);
            const double mag = is_negative ? -scaled : scaled;
            ...
            for (std::size_t i = 0; i < NUM_PRIMES; ++i) {
                ...
                out.residues[i][j] = is_negative ? (r == 0 ? 0 : q - r) : r;
            }
        }
        return out;
    }
    ...
    std::vector<std::complex<double> > Encoder::decode(const Polynomial &p, double scale, std::size_t level) const {
        constexpr std::size_t N = POLY_DEGREE;
        constexpr std::size_t H = N / 2;
        if (!(scale > 0.0)) {
