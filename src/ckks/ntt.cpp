...
namespace ssns::ckks {
    ...
    namespace {
        ...
        void forward_typed(std::uint64_t *a, std::size_t N,
            ...
            std::size_t m = 1;
            for (std::size_t s = N / 2; s >= 1; s >>= 1) { // s - расстояние связанных элементов
    ...
    void NTT::forward(std::uint64_t *a) const {
        ...
        if (p_ == COEFF_MODULI[0]) {
            forward_typed<COEFF_MODULI[0], PSM_C[0], true>(a, N_, psi);
            return;
        } else if (p_ == COEFF_MODULI[3]) {
            forward_typed<COEFF_MODULI[3], PSM_C[3], true>(a, N_, psi);
            return;
        } else if (p_ == COEFF_MODULI[1]) {
            forward_typed<COEFF_MODULI[1], PSM_C[1], false>(a, N_, psi);
            return;
        } else if (p_ == COEFF_MODULI[2]) {
            forward_typed<COEFF_MODULI[2], PSM_C[2], false>(a, N_, psi);
            return;
        }
    ...
    void NTT::inverse(std::uint64_t *a) const {
        ...
        if (p_ == COEFF_MODULI[0]) {
            inverse_typed<COEFF_MODULI[0], PSM_C[0], true>(a, N_, inv_psi, inv_N_);
            return;
        } else if (p_ == COEFF_MODULI[3]) {
            inverse_typed<COEFF_MODULI[3], PSM_C[3], true>(a, N_, inv_psi, inv_N_);
            return;
        } else if (p_ == COEFF_MODULI[1]) {
            inverse_typed<COEFF_MODULI[1], PSM_C[1], false>(a, N_, inv_psi, inv_N_);
            return;
        } else if (p_ == COEFF_MODULI[2]) {
            inverse_typed<COEFF_MODULI[2], PSM_C[2], false>(a, N_, inv_psi, inv_N_);
            return;
        }
