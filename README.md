...
1. **All neural-net math is hand-written.**  No PyTorch, no Eigen.  The only
   third-party numerical hook is `cblas_dgemm` for matmul (replaceable with a
   self-written SIMD kernel in a later phase).
...
Requires: C++20 compiler, CMake >= 3.20, OpenBLAS runtime (libopenblaso.so).
